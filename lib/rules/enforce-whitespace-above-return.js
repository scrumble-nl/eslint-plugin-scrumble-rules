...
module.exports = {
    ...
    create: context => {
        ...
        function getStatementContainer(node) {
            ...
            if (parentNode.type === 'BlockStatement') return parentNode.body;
            if (parentNode.type === 'SwitchCase') return parentNode.consequent;

            return null;
        }

        function isOnlyStatementInContainer(node) {
            const container = getStatementContainer(node);

            if (!container) return false;

            const statements = container.filter(
                statement => statement && statement.type !== 'EmptyStatement',
            );

            return statements.length === 1 && statements[0] === node;
        }
        ...
        return {
            ReturnStatement(node) {
                if (
                    isOnlyStatementInContainer(node) ||
